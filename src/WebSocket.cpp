...
void WebSocketUtil::createThread(const WebSocket* ws)
{
...
void WebSocketUtil::updateHandler(float delta)
{
    ...
    {
        _UIWebSocketMessageQueueMutex->unclock();
        return;
    }
...
bool WebSocket::connect(const string& url, vector<string>* subProtocols)
{
    parseUrl(url);
    ...
    return util->createWebSocketThread(*this);
}
...
int WebSocket::onSocketCallback(struct libwebsocket_context *ctx,
                                struct libwebsocket_contex *wsi,
                                int reason,
                                void *user,
                                void *in,
                                size_t len)
        ...
        case LWS_CALLBACK_CLIENT_RECEIVE:
            {
                if ( in && len > 0 )
                {
                    ...
                    _pendingFrameDataLength = libwebsocket_remaining_packet_payload(wsi);
